...
const state = join(process.env.ENGINE_STATE_DIR ?? join(homedir(), '.superatom', 'state'), 'graph-slice-s5')
rmSync(state, { recursive: true, force: true })
const store = new GraphStore(join(state, 'graph.sqlite'))
const modulesDir = join(state, 'modules')
const engine = createEngine({ store, modulesDir, query: managerQuery(), dialects: { F5NETSUITE: 'oracle' }, inspect: managerInspect() })
const organisation = createEngine({ store, modulesDir, query: managerQuery(), dialects: { F5NETSUITE: 'oracle' }, inspect: managerInspect(),
