...
class ExampleTable extends HTMLElement {
  connectedCallback() {
    ...
    this.innerHTML = `
      <div class="sa-card">
        <div class="sa-pad-t">
          <div class="sa-label">${d.title || 'Data'}</div>
          ${d.subtitle ? `<div class="sa-sublabel">${d.subtitle}</div>` : ''}
        </div>
        <table class="sa-table">
          <thead><tr>${cols.map(c => `<th class="${c.align || ''}">${c.label}</th>`).join('')}</tr></thead>
          <tbody>
            ${rows.map(row => `<tr>${cols.map(c => {
              const v = row[c.key];
              const cls = [c.align, c.mono ? 'mono' : '', c.strong ? 'strong' : ''].filter(Boolean).join(' ');
              const badge = row[c.key + '_badge'];
              return `<td class="${cls}">${badge ? `<span class="sa-badge sa-badge-${badge.type}">${badge.text}</span> ` : ''}${v ?? '—'}</td>`;
            }).join('')}</tr>`).join('')}
          </tbody>
        </table>
...
customElements.define("example-table", ExampleTable);
