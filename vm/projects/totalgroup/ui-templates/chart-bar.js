...
// Template: chart-bar
## Template — Bar chart (ECharts)

class ExampleChart extends HTMLElement {
  connectedCallback() {
    const d = this._data || {};
    const uid = "c-" + Math.random().toString(36).slice(2, 7);
    this.innerHTML = `
      <div class="sa-card sa-pad">
        <div class="sa-label">${d.title || 'Chart'}</div>
    ...
    chart.setOption({
      ...
      xAxis: { type: "category", data: d.categories || [], axisLine: { lineStyle: { color: "#e8e4de" } }, axisTick: { show: false }, axisLabel: { color: "#9a9285", fontSize: 11 } },
      ...
      series: (d.series || []).map((s, i) => ({
        name: s.name, type: "bar", stack: s.stack || undefined, barMaxWidth: 36,
        data: s.data || [],
        itemStyle: { color: s.color || ["#e55a1f","#059669","#2563eb","#d97706"][i % 4], borderRadius: [4,4,0,0] },
        label: s.showLabel ? { show: true, position: "top", color: s.color || "#e55a1f", fontWeight: 600, fontSize: 11, formatter: (p) => p.value } : { show: false }
      }))
    });
  }
}
