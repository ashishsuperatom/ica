...
// Template: prose
## Template — Prose (a data-driven sentence/paragraph; reads this._data and stays correct on data change)
## Use this INSTEAD of a plain text item whenever the sentence states specific values, names, or insights.

class ExampleProse extends HTMLElement {
  connectedCallback() {
    const d = this._data || {};
    // Compute the narrative FROM the data — filter/sort/pick as needed (read YOUR real keys).
    // e.g. const top = (d.rows || [])[0]; const losers = (d.rows || []).filter(r => r.value < 0);
    // Render *_display fields verbatim; never build number/currency strings yourself.
    const sentence = ''; // build the sentence from d here
    this.innerHTML = `<div class="sa-card sa-pad"><p style="margin:0;line-height:1.6;color:var(--sa-text)">${sentence}</p></div>`;
  }
}
customElements.define("example-prose", ExampleProse);
