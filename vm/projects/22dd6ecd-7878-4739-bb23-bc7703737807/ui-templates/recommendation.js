...
## Template — Action / recommendation card (table rows with priority badge)

class ExamplePlan extends HTMLElement {
  connectedCallback() {
    const d = this._data || {};
    ...
    this.innerHTML = `
      <div class="sa-card">
        <div class="sa-pad-t">
          <div class="sa-label">${d.label || 'Recommended Actions'}</div>
          ${d.title ? `<div class="sa-title">${d.title}</div>` : ''}
          ${d.subtitle ? `<div class="sa-sublabel">${d.subtitle}</div>` : ''}
        </div>
        <table class="sa-table">
          <thead><tr>
            <th>Action</th>
            <th>Detail</th>
            ${d.showValue ? '<th class="right">Value</th>' : ''}
            <th class="right">Priority</th>
          </tr></thead>
          <tbody>
            ${actions.map(a => `<tr>
              <td class="strong nowrap">${a.action}</td>
              <td class="muted">${a.detail}</td>
              ${d.showValue ? `<td class="right strong">${a.value || '—'}</td>` : ''}
              <td class="right">
                <span class="sa-badge ${a.priority === 'High' ? 'sa-badge-red' : a.priority === 'Medium' ? 'sa-badge-amber' : 'sa-badge-blue'}">${a.priority}</span>
              </td>
            </tr>`).join('')}
          </tbody>
        </table>
        ${d.summary ? `<div class="sa-summary"><span class="muted">${d.summary.label}</span><span class="sa-summary-val ${d.summary.color || ''}">${d.summary.value}</span></div>` : ''}
      </div>
    `;
  }
}
customElements.define("example-plan", ExamplePlan);
