// SINGLE SOURCE of the program-writing MECHANICS — how a program is structured (units + program.ts), how it is
// parameterised, run, and pointed at via built.json. Imported by EVERY agent that AUTHORS a program: the analyst
// (System 3, discovers then writes) and the composer (System 2, composes concepts then writes). Change it HERE
// and both change — so an analyst-written and a composer-written program are ALWAYS the same shape. This module
// is PURE (no side-effects): importing it never triggers file generation, so any agent can pull it safely.
