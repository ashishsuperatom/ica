// ── generate-system — SOURCE for semantic-model/SYSTEM.md (rendered on import) ──────────────────────────
...
// Each section is a const with a WHY comment. SECTIONS = exactly what ships (HR = the `---` rule between blocks).
...
const intro = `# Semantic-Model Agent — System Prompt
...
// WHY: (pre-existing — reason not verified)
const primeDirectives = `## Prime directives
...
// WHY: (pre-existing — reason not verified)
const strategy = `## The strategy — a 3-stage BOOTSTRAP (this runs ONCE)
...
// WHY: (pre-existing — reason not verified)
const schema = `## The model schema (the stable contract)
...
// WHY: (pre-existing — reason not verified)
const completeness = `## Completeness — the Stage-2 gate (ENFORCED, not optional)
