...
export function isDataCall(command?: string): boolean {
  ...
  return /\b(query|introspect|resolve|find-concept|get-concept|find-schema|sources|run\.mjs|program\.ts|tsx|node)\b/.test(c)
}
...
async function analyse(question: string, from: any, sid = '', qidIn = '', channel = '') {
  ...
  // NO LOOKUP HERE. A program belongs to the conversation that asked for it, so there is nothing to look up
  ...
  // its own directory — so what it worked from, the run and
  ...
  if (explicitEdit) {
    ...
    // NO CROSS-CONVERSATION PROGRAM SEARCH. The engine used to hand the composer a shortlist of programs that
    // had answered similar questions, drawn from every conversation on the project. It goes for two reasons.
    //
    // A program encodes the scope, filters and assumptions of the question that produced it, so it belongs to
    // the conversation that asked for it and to nobody else. Concepts are the shared layer, and they are
    // verified; a program is working material.
    //
    // And the search ran on the raw wording, which made it worst exactly when it mattered: asked "based on past
    // data can you project it to future", it returned a program that lists datasources at 0.74 — a confident
    // number pointing at the wrong neighbourhood — because pronouns match generic words. What a conversation
    // built for itself is already in the composer's own directory and its own context.
  ...
  try {
    ...
    startNarrator()
    {
      ...
      const cAsk = composer.ask(question, handlers, { qid, sid, modify: modifyTarget ?? undefined, resolvedQuestion })
    ...
    if (b) { programDir = b.programDir; programParams = b.params; programTerms = Array.isArray(b.terms) ? b.terms : []
             programFollowups = Array.isArray(b.followups) ? b.followups.filter((x: any) => typeof x === 'string' && x.trim()).slice(0, 3) : [] }
    ...
    if (authoredProgramDir && builtIntentId) {
      ...
      // The node records WHAT WAS BUILT and by whom — provenance the modeller reads when it consolidates.
      // It no longer carries question forms: those existed so one conversation's program could be found and
      // re-run for another, and a program encodes the scope of the question that produced it.
...
// Kill the engine mid-turn and the directory is already on disk with no node — invisible to the engine,
// but plainly visible to `ls`. So the agent sees a directory that looks like an
