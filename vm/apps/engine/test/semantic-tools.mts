...
assert.equal(JSON.parse(await tool('match', '--json', 'hours in October 2026')).terms.terms[0].phrase, 'hours')
// The agent runs tools side by side, and the engine shares their memory: none is refused for a locked database.
const together = await Promise.all(Array.from({ length: 6 }, () => tool('look', 'Sale')))
...
assert.match(await tool('look', 'Sale'), /^\(:Sale\) fact/)
assert.equal(JSON.parse(await tool('look', 'Sale', '--json')).node.name, 'Sale')
assert.match(await tool('look', 'Sale'), /\(:Sale\) is sliced by \d+ dimensions/)
console.log('help:', (await tool('ask', '--help')).slice(0, 80))
console.log('match hours:', (await tool('match', 'hours by branch in October 2026')).replace(/\s+/g, ' ').slice(0, 160))
console.log('look Branch sydney:', (await tool('look', 'Branch', 'sydney')).replace(/\s+/g, ' ').slice(0, 160))
console.log('look Sale Branch:', (await tool('look', 'Sale', 'Branch')).replace(/\s+/g, ' '))
const refused = (await tool('ask', JSON.stringify({ measures: ['Sale.hours'], by: [{ to: 'Branch' }] })))
console.log('check (ambiguous):', refused.replace(/\s+/g, ' ').slice(0, 220))
assert.match(refused, /"rule": "A2"/)
const tried = (await tool('ask', JSON.stringify({ measures: ['Sale.hours'], by: [{ to: 'Branch', via: ['project', 'branch'] }], span: { from: '2026-09-01', through: '2026-10-31' } })))
