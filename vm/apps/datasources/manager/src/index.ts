...
function compilePrql(prql: string, dialect?: string): string {
  ...
  const src = /^\s*prql\s+target:/.test(prql) ? prql : `prql target:${target}\n${prql}`
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (url.pathname === '/query') {
      ...
      // PRQL branch: the query text IS PRQL — compile it to the source's SQL dialect, then run. One place, so
      // the seam AND the program kernel both get PRQL for free. `sql` (the compiled SQL) is returned for visibility.
      const sql = compilePrql(body.sql, bridge.dialect)
      return send(res, 200, { rows: await bridge.query(sql, body.params ?? {}), sql })
    }
